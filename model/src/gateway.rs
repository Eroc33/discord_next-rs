...
impl Payload {
    pub fn try_from_command<P: Into<GatewayCommand>>(
        payload: P,
        ...
        let (op, payload) = match payload {
            ...
            StatusUpdate(status_update) => {
                (opcode::STATUS_UPDATE, serde_json::to_value(status_update)?)
            }
            #[cfg(feature = "voice")]
            VoiceStateUpdate(voice_status_update) => (
                opcode::VOICE_STATE_UPDATE,
                serde_json::to_value(voice_status_update)?,
            ),
            Resume(resume) => (opcode::RESUME, serde_json::to_value(resume)?),
            RequestGuildMembers(request_guild_members) => (
                opcode::REQUEST_GUILD_MEMBERS,
                serde_json::to_value(request_guild_members)?,
            ),
        };
        Ok(Payload {
            op,
...
impl TryFrom<Payload> for GatewayEvent {
    ...
    fn try_from(payload: Payload) -> Result<Self, Self::Error> {
        Ok(match payload.op {
            ...
            opcode::INVALID_SESSION => GatewayEvent::InvalidSession(InvalidSession {
                resumable: serde_json::from_value(payload.d)?,
            }),
            ...
            other if known_opcode(other) => {
                panic!("should never recieve `{}` payload", opcode_name(other))
            }
...
wrapping_from!(
    ReceivableEvent,
    GuildEmojisUpdate,
    expect_guild_emojis_update
);
wrapping_from!(
    ReceivableEvent,
    GuildIntegrationsUpdate,
    expect_guild_integrations_update
);
wrapping_from!(ReceivableEvent, GuildMemberAdd, expect_guild_member_add);
wrapping_from!(
    ReceivableEvent,
    GuildMemberRemove,
    expect_guild_member_remove
);
wrapping_from!(
    ReceivableEvent,
    GuildMemberUpdate,
    expect_guild_member_update
);
wrapping_from!(
    ReceivableEvent,
    GuildMembersChunk,
    expect_guild_members_chunk
);
...
wrapping_from!(
    ReceivableEvent,
    MessageDeleteBulk,
    expect_message_delete_bulk
);
wrapping_from!(
    ReceivableEvent,
    MessageReactionAdd,
    expect_message_reaction_add
);
wrapping_from!(
    ReceivableEvent,
    MessageReactionRemove,
    expect_message_reaction_remove
);
wrapping_from!(
    ReceivableEvent,
    MessageReactionRemoveAll,
    expect_message_reaction_remove_all
);
